...
class Session {
    ...
    private function render(): void {
        ...
        $this->tui->write(str_repeat("─", $w - 2), TUI::CYAN);
        $this->tui->write("┐", TUI::CYAN);

        $this->tui->move(2, 2);
        $this->tui->write("OllamaDev", TUI::GREEN, true);
        $this->tui->write(" | ", TUI::DIM);
        $this->tui->write($this->model, TUI::CYAN);
        $this->tui->write(" | " . count($this->messages) . " msgs", TUI::DIM);
        ...
        // If overlay is shown, draw panel on left side
        if ($this->showModels && !empty($this->models)) {
            $panelW = 45;
            $panelH = min(count($this->models) + 5, $h - 8);
            $this->tui->box($contentTop, 2, $panelH, $panelW, "Models (Esc close)");
            $row = $contentTop + 2;
            foreach ($this->models as $m) {
                ...
                $sel = $name === $this->model ? " ◀" : "";
                $this->tui->move($row++, 4);
                $this->tui->write(sprintf("%-25s %s%s", $name, $size, $sel), $sel ? TUI::GREEN : TUI::WHITE);
            }
            $contentTop += $panelH + 1;
        }
        ...
        if ($this->showSessions && !empty($this->sessions)) {
            $panelW = 45;
            $panelH = min(count($this->sessions) + 5, $h - 8);
            $this->tui->box($contentTop, 2, $panelH, $panelW, "Sessions (Esc close)");
            $row = $contentTop + 2;
            foreach ($this->sessions as $s) {
                $title = substr($s['title'] ?? 'Untitled', 0, 30);
                $this->tui->move($row++, 4);
                $this->tui->write($title, TUI::WHITE);
            }
            $contentTop += $panelH + 1;
        }
        ...
        if ($this->showHelp) {
            $panelW = 45;
            $panelH = 15;
            $this->tui->box($contentTop, 2, $panelH, $panelW, "Help");
            $cmds = [
                ["Ctrl+C", "Quit"],
                ["Ctrl+O", "Models"],
                ["Ctrl+N", "New Session"],
            ...
            foreach ($cmds as $c) {
                $this->tui->move($row++, 4);
                $this->tui->write(str_pad($c[0], 10), TUI::CYAN, true);
                $this->tui->write($c[1], TUI::DIM);
            }
            $contentTop += $panelH + 1;
        }

        // Messages area
        $msgAreaH = $h - $contentBottom - $contentTop - 1;
        $this->tui->renderMessages($this->messages, $contentTop, $contentBottom);

        // Separator line
        $sep = $h - $contentBottom - 1;
        $this->tui->move($sep, 1);
        $this->tui->write("└", TUI::CYAN);
        $this->tui->write(str_repeat("─", $w - 2), TUI::CYAN);
        $this->tui->write("┘", TUI::CYAN);
        ...
        // Status bar
        $this->tui->statusBar("[C-c] Quit  [C-o] Models  [C-n] New  [C-s] Sessions  [C-k] Help", "", $h);
    }
