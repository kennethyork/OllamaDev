...
class PtyManager
{
    ...
    {
        // Explicit override always wins (the shipped archive's launchers set this
        // to the bundled bin/ollamadev, so packaged apps never reach the rest).
        if (defined('OLLAMADEV_BINARY') && OLLAMADEV_BINARY) return OLLAMADEV_BINARY;
        if ($env = getenv('OLLAMADEV_BINARY')) return $env;
        $home = getenv('HOME') ?: '';
        // Running from a source checkout? Prefer the repo's freshly built binary
        // over anything on PATH, so the desktop/web see new CLI features after a
        // `./build.sh` without needing to reinstall. (A `build.sh` next to it is
        // the dev-checkout signature; shipped archives never match this.)
        $repoRoot = dirname(__DIR__, 3);
        $candidates = [];
        if (is_file($repoRoot . '/ollamadev') && is_file($repoRoot . '/build.sh')) $candidates[] = $repoRoot . '/ollamadev';
        $candidates[] = $home . '/.local/bin/ollamadev';
        $candidates[] = trim((string)@shell_exec('command -v ollamadev 2>/dev/null'));
        $candidates[] = $repoRoot . '/ollamadev';   // fallback: built binary without the dev signature
