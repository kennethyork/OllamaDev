...
#
# Usage:
#   scripts/setup-vps.sh                 # qwen3.5:9b (~6.6 GB, tool-capable — matches local crew)
#   scripts/setup-vps.sh --ctx 4096      # override the context cap (default 8192)
#
# Why qwen3.5:9b: it reports a `tools` capability and fits in 24 GB with room to
# spare, so one model can back every crew role (no swap thrash). The 22 GB+ models
# (qwen3.6, *:32b, *:35b) do NOT fit alongside anything on 24 GB and are painfully
# slow on CPU, so this script refuses to use them.
set -euo pipefail
cd "$(dirname "$0")/.."
