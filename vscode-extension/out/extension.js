...
function getConfig() {
    ...
    return {
        port: cfg.get('port', 4389),
        hostname: cfg.get('hostname', '127.0.0.1'),
        autoStart: cfg.get('autoStart', true),
        model: cfg.get('model', 'llama3.2:latest'),
        inlineCompletionEnabled: cfg.get('inlineCompletionEnabled', false),
        statusBarEnabled: cfg.get('statusBarEnabled', true)
    };
}
...
async function aiRequest(method, params) {
    try {
...
class InlineCompletionProvider {
    ...
    async provideInlineCompletionItems(document, position, context, token) {
        if (!this.enabled && !getConfig().inlineCompletionEnabled)
            return null;
        ...
        try {
            ...
            if (result && result.text) {
                ...
                return [new vscode.InlineCompletionItem(new vscode.SnippetString(result.text), undefined, { command: 'ollamadev.complete.accepted', title: 'OllamaDev AI' })];
            }
        }
        catch (err) {
    ...
    async getStreamingCompletion(code, cursor, token) {
        return new Promise((resolve, reject) => {
            let result = '';
            let resolved = false;
...
async function getCompletion() {
    ...
    try {
        ...
        vscode.window.showErrorMessage('Completion failed: ' + err);
    }
}
function toggleInlineCompletion(provider) {
