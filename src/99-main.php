...
if ($argc >= 2 && $argv[1] === 'ship') {
    ...
    if (!$tty) { echo "Committed. Re-run in a terminal to push — ship asks before it pushes.\n"; exit(0); }
...
Commands:
  ...
  ollamadev ship        Stage all → scan secrets → AI commit → ask before pushing (one command)
