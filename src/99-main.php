...
if ($argc >= 2 && $argv[1] === 'chat') {
    ...
    $pendingImages = [];   // images attached via /image, sent with the next message
    ...
    while (true) {
        ...
        if ($t === '/help' || $t === '/?') {
            ...
            echo $c("  /image <path>    attach an image     /clear          start a fresh conversation\n", '2');
            echo $c("  /exit            leave chat          Ctrl-D          leave chat\n", '2');
            continue;
        }
        if ($t === '/clear' || $t === '/new' || $t === '/reset') { $history = [['role' => 'system', 'content' => $system]]; $pendingImages = []; echo $c("— new conversation —\n", '2'); continue; }
        ...
        // /image <path> — attach an image to the NEXT message (for a vision model).
        if (strncmp($t, '/image', 6) === 0) {
            $path = trim(substr($t, 6));
            if ($path === '') { echo $c("usage: /image <path-to-image>\n", '2'); continue; }
            if ($path !== '' && $path[0] === '~') $path = $home . substr($path, 1);
            if (!is_file($path)) { echo $c("✗ no such file: $path\n", '33'); continue; }
            $data = @file_get_contents($path);
            if ($data === false || $data === '') { echo $c("✗ could not read: $path\n", '33'); continue; }
            $pendingImages[] = base64_encode($data);
            echo $c('📎 attached ' . basename($path) . " — now type your message (it'll go with the image).\n", '2');
            if (!OllamaClient::modelSupportsVision($model)) echo $c("  ⚠ \"$model\" may not support images — try a vision model (e.g. llava, qwen2.5-vl).\n", '33');
            continue;
        }
        $userMsg = ['role' => 'user', 'content' => $line];
        if ($pendingImages) { $userMsg['images'] = $pendingImages; $pendingImages = []; }   // consume the attachment
        $history[] = $userMsg;
        echo $c("\n" . $model . " ▸ ", '1;36');
