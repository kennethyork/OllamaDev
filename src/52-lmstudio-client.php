...
class LMStudioClient {
    ...
    private function body(string $model, array $messages, array $extra = []): string {
        return json_encode(array_merge([
            ...
            'temperature' => (float)Config::get('ollama.temperature', 0.3),
        ], $extra));
    }
    ...
    // Schema-constrained decoding via OpenAI-style json_schema response format
    // (LM Studio supports this). Mirrors OllamaClient::chatStructured.
    public function chatStructured(string $model, array $messages, array $schema): ?array {
        $ch = curl_init($this->host . '/chat/completions');
        curl_setopt_array($ch, [
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $this->body($model, $messages, ['stream' => false,
                'response_format' => ['type' => 'json_schema', 'json_schema' => ['name' => 'tool_response', 'strict' => true, 'schema' => $schema]]]),
            CURLOPT_HTTPHEADER => ['Content-Type: application/json', 'Authorization: Bearer lm-studio'],
            CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 300,
        ]);
        $resp = curl_exec($ch);
